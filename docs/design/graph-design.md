...
| 16 | `update_memory` | Apply the long-term memory write policy (defined in `memory-design.md`) and upsert the customer profile. Runs in parallel with #17. | Cheap model | — | `final_output`, `ticket`, `clarifications`, `messages` (customer-named: rejection feedback) | — (external write only) |
