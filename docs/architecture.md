...
**Implementation:** a virtual table `dataset_tickets_fts` over `subject`, `body`, `answer` and the joined tag string, kept in sync with `dataset_tickets` and `cases`. Both retrieval arms return ranked ID lists, which are fused with **Reciprocal Rank Fusion** in `autosupport/rag/fusion.py` — roughly forty lines, written in-repo rather than pulled from a library, so the ranking is fully inspectable. Query construction, `k`, the RRF constant and MMR are specified in `rag-design.md`.
...
- `dataset_tickets`: a read-only mirror of the ingested HF rows (`HF-<row>`). It lets `get_ticket_by_id` and `compute_queue_stats` run as SQL without going through the vector store. Beyond the source columns it carries the ingest-time fields `answer_class`, `cluster_id`, `cluster_size`, `is_canonical` and `canonical_of`.
...
1. **Ingest (once):** `autosupport ingest` runs six stages. Full parameters are in `rag-design.md`; this is the contract between them.
   1. **Load and filter.** Pull the Hugging Face dataset, keep `language == "en"`, snapshot to local parquet so later runs are offline and reproducible. Each row gets the deterministic ID `HF-<row_index>`.
   2. **Classify answers.** Assign `answer_class` ∈ {`resolution`, `clarification_request`, `escalation`}. Heuristic and regex first pass, precision measured against a hand-labelled random sample of 200, and an LLM applied only to the ambiguous residue. This deliberately avoids 28K model calls; the sampled precision figure is reported in the README as methodology rather than asserted.
   3. **Cluster and canonicalise.** Normalise text, group near-duplicates, and mark one canonical per cluster. Every row records `cluster_id`, `cluster_size`, `is_canonical` and `canonical_of`.
   4. **Write SQLite.** All rows land in `dataset_tickets`, canonicals and members alike, so a member is still reachable by ID.
   5. **Build the lexical index.** Populate `dataset_tickets_fts` (§2.1b).
   6. **Embed and upsert.** Only canonicals are embedded. Embed text is subject + body + selected metadata; the Chroma record carries `answer_class`, `cluster_size` and the tag keys alongside queue, type and priority.
...
- **`answer_class` is heuristic, not ground truth.** The labels come from pattern matching validated against a 200-row sample, with an LLM only on ambiguous cases. The README reports the measured precision; residual mislabelling will occasionally send a resolvable ticket down the clarification route or the reverse.
- **Clustering is threshold-based.** A near-duplicate threshold is a judgement call. Set it too tight and `cluster_size` understates real agreement; too loose and genuinely distinct cases get merged, inflating confidence. The chosen threshold and its sensitivity are recorded in `rag-design.md`.
- **Only canonicals are retrievable by similarity.** Cluster members exist in SQLite and can be fetched by ID, but they never surface from a search. This is intentional — it is what stops top-k from returning the same case five times — but it means the vector index is smaller than the stated corpus size.
