"""The rules behind `assess_evidence`: which cases are relevant, how
...
QUEUE_AGREEMENT_FLOOR = 0.5  # only triggers retrieval variant V4 now, not a verdict
FIX_CLUSTER_FLOOR = 0.5  # a cluster "proposes a fix" if at least half its weight is resolution-class

# `critical` priority alone isn't enough: it has to point at one of
...
def top_queue_share(relevant: list[RetrievedCase]) -> float:
    ...
    refinement could otherwise never move."""
...
def dominant_cluster(
    ...
    ("15 cases against 3" reads differently from "15 against 14")."""
...
def verdict_for(
    *, relevant: list[RetrievedCase], tau_rel: float, clusters: list[ApproachCluster],
    ...
    """Checked insufficient -> conflicting -> sufficient. Returns
    ...
    the model's opinion of the evidence, which can disagree with the verdict."""
    ...
    # fix it, so it never counts as enough to resolve.
    if dom and _answer_class_share(dom.case_ids, relevant, "clarification_request") >= CLARIFICATION_SHARE_FLOOR:
        return "insufficient", share, dom, "similar tickets were mostly answered with questions, not fixes"

    # Support and conflict are both judged among clusters that propose a *fix*. Clusters
    # of "asked for more info" or "escalated" answers aren't approaches: counting support in
    # the heaviest cluster overall once found 0 supporters, because the investigator had put
    # the escalated leftovers in one catch-all group that outweighed a 4-case fix. The
    # neighbours' queue labels are too noisy to signal conflict either: 3 of 5 resolution-class
    # eval tickets had an all-resolution dominant cluster yet were called conflicting by them.
    ...
    lead = fixes[fix_weight.index(max(fix_weight))]
    supporting = sum(1 for e in evidence if e.stance == "supports" and e.case_id in set(lead.case_ids))
    if supporting < MIN_SUPPORTING_IN_DOMINANT:
        return "insufficient", share, dom, (
            f"only {supporting} case(s) support the most common fix; {MIN_SUPPORTING_IN_DOMINANT} needed")
...
def escalation_rule_hit(
    *, classification: Classification, dom: ApproachCluster | None, relevant: list[RetrievedCase],
    ...
    """First matching rule name, stored on the assessment and later
...
MAX_MISSING_SLOTS = 2
# Never ask a customer to send these in a ticket, whatever the model proposes (measured:
...
# under-specified ticket (the demo's "my app stopped syncing" ticket was escalated
...
def next_action_for(
    *, verdict: Verdict, rule_hit: str | None, gap_is_retrievable: bool, missing_slots: list[str],
    ...
    """A matched escalation rule wins outright: it names work a human
    must do, so asking the customer or searching again first only delays the handoff (an early eval
    found rule-hit tickets asking instead). Then retrieval before asking the user:
    re-retrieval is cheap and asks nothing of the customer. Every branch that can't progress
    lands on `escalate`, so the loop always terminates."""
    if rule_hit:
        return "escalate"
